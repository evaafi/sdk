...
import { EvaaMasterClassic, FEES, MAINNET_V8_TOB_POOL_CONFIG, USDT_MAINNET } from '../src';
...
async function supplyJetton() {
    ...
    const EVAA_TESTNET = TON_CLIENT.open(
        new EvaaMasterClassic({ poolConfig: MAINNET_V8_TOB_POOL_CONFIG, debug: true }),
    );
    const WALLET_SENDER = {
    ...
    await EVAA_TESTNET.sendSupply(WALLET_SENDER, FEES.SUPPLY_WITHDRAW_JETTON, {
        queryID: 0n,
