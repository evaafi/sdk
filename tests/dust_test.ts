...
import { ClassicCollector, Evaa, JUSDC_MAINNET, USDT_MAINNET } from '../src';
...
test('Manual dust check', async () => {
    ...
    await evaa.getSync();
    const user = client.open(
        await evaa.openUserContract(Address.parseFriendly('UQDN5CpSs8HT2GO4IymOXPS5zTDzHtY-s8VTuUVAsCTwWJzM').address),
    );

    const collector = new ClassicCollector(MAINNET_STABLE_POOL_CONFIG);
    ...
    console.log(
        ...
        (await collector.getPricesForWithdraw(user.data.realPrincipals, USDT_MAINNET)).dict,
    );
