...
import { EvaaMasterPyth, TESTNET_PYTH_POOL_CONFIG_TOB_AUDITED, TON_TESTNET } from '../src';
...
async function supplyJetton() {
    ...
    const EVAA_TESTNET = TON_CLIENT.open(
        new EvaaMasterPyth({ poolConfig: TESTNET_PYTH_POOL_CONFIG_TOB_AUDITED, debug: true }),
    );
    const WALLET_SENDER = {
